...
try
    ...
    for ii=1:size(PsfragCmds,1)
        ...
        fprintf(fid,'{\\footnotesize %s}%%',RemoveSpaces(PsfragCmds{ii,1}));
    end
    fprintf(fid,'\n%%\n%%%% </%s>',CurrentType);
    %     moj dodatek, legenda
    legendHandle=findobj(p.Results.handle, 'Type', 'legend');
    if ~isempty(legendHandle)
        
        for i=1:1:length(legendHandle.String)
            fprintf(fid,'\n\\psfrag{%s}[cl][cl]{\\footnotesize %s}%%',legendHandle.String{i},legendHandle.String{i});
        end
    end
    fclose(fid);
